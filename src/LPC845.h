...
typedef struct {
  __IO uint8_t B[2][32];				/**< Byte pin registers*/
       uint8_t RESERVED_0[4032];		/**<RESERVATED. NOT USED */
  __IO uint32_t W[2][32];				/**< Word pin registers*/
       uint8_t RESERVED_1[3840];		/**<RESERVATED. NOT USED */
  __IO uint32_t DIR[2];					/**< Direction registers*/
       uint8_t RESERVED_2[120];			/**<RESERVATED. NOT USED */
  __IO uint32_t MASK[2];				/**<  Mask register*/
       uint8_t RESERVED_3[120];			/**<RESERVATED. NOT USED */
  __IO uint32_t PIN[2];					/**< Port pin register*/
       uint8_t RESERVED_4[120];			/**<RESERVATED. NOT USED */
  __IO uint32_t MPIN[2];				/**< Masked port register*/
       uint8_t RESERVED_5[120];			/**<RESERVATED. NOT USED */
  __IO uint32_t SET[2];					/**< Write: Set register for port .Read: output bits for port */
       uint8_t RESERVED_6[120];			/**<RESERVATED. NOT USED */
  __O  uint32_t CLR[2];					/**<  Clear port */
       uint8_t RESERVED_7[120];			/**<RESERVATED. NOT USED */
  __O  uint32_t NOT[2];					/**< Toggle port*/
       uint8_t RESERVED_8[120];			/**<RESERVATED. NOT USED */
  __O  uint32_t DIRSET[2];				/**< Set pin direction bits for port*/
       uint8_t RESERVED_9[120];			/**<RESERVATED. NOT USED */
  __O  uint32_t DIRCLR[2];				/**< Clear pin direction bits for port*/
       uint8_t RESERVED_10[120];		/**<RESERVATED. NOT USED */
  __O  uint32_t DIRNOT[2];				/**< Toggle pin direction bits for port*/
...
typedef struct {
  __IO uint32_t PIO[56];			/**< I/O configuration for pin [X]*/
...
typedef	struct
{
	...
	union{
		struct{
			...
			__I		uint32_t	RESERVED0:1;	/**<RESERVATED. NOT USED */
		};
		__I		uint32_t	TIMER;
	};
	union{
		struct{
			...
			__IO	uint32_t	RESERVED1:29;	/**<RESERVATED. NOT USED */
		};
		__IO	uint32_t	CTRL;
	};
	union{
		struct{
			...
			__IO	uint32_t	RESERVED2:30;	/**<RESERVATED. NOT USED */
		};
		__IO	uint32_t	STAT;
	};
...
typedef struct {
	...
	union {
		struct{
			__IO uint32_t RESERVED0	:6;				/**< RESERVATED. NOT USED */
			__IO uint32_t VALUE		:10;			/**< After the selected settling time after this field is written with a new VALUE, the voltage on the DAC_OUT pin (with respect to VSSA) is VALUE = ((VREFP)/1024)*/
			__IO uint32_t BIAS		:1;				/**< The settling times noted in the description of the BIAS bit are valid for a capacitance load on the DAC_OUT pin not exceeding 100 pF*/
			__IO uint32_t RESERVED1	:15;			/**< RESERVATED. NOT USED */
		};
		__IO uint32_t CR;							/**< This read/write register includes the digital value to be converted to analog, and a bit that trades off performance vs. power.*/
	};
	union{
		struct{
			__IO uint32_t INT_DMA_REQ	:1;			/**< DMA interrupt request. 0=Clear on any write to the CR register. 1=Set by hardware*/
			__IO uint32_t DBLBUF_ENA	:1;			/**< Double buffering. 0=Disable. 1=Enable*/
			__IO uint32_t CNT_ENA		:1;			/**< Time-out counter operation. 0=Disable. 1=Enable*/
			__IO uint32_t DMA_ENA		:1;			/**< Enable. DMA Burst Request Input 7 is enabled for the DAC*/
			__IO uint32_t RESERVED2		:28;		/**< RESERVATED. NOT USED */
		};
		__IO uint32_t CTRL;							/**< This read/write register enables the DMA operation and controls the DMA timer*/
	};

	__IO uint32_t CNTVAL;						/**< 16-bit reload value for the DAC interrupt/DMA timer.*/
